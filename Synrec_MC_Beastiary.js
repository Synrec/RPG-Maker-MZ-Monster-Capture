 ...
 * @target MZ
 *
 * @plugindesc v1.4 Creates a simple scene which allows actor evolution
 * 
 * @help
 ...
 * 
 * This plugin only allows category renaming.
 * 
...
SynrecMC.Beastiary = {};
SynrecMC.Beastiary.Version = "1.4";
...
SynrecMC.Beastiary.ListFilterName = SynrecMC.Beastiary.Plugins['List Name Filter'];
...
Scene_Beastiary.prototype.createCommandWindow = function(){
    ...
    const y = 0;
    const width = Graphics.width - (Graphics.width / 3);
    const height = Graphics.height / 9;
    ...
    switch(this._mode){
        ...
        default:
            ...
            for(i = 0; i < $dataActors.length; i++){
                ...
                let dataId = this._dataIndices[i];
                if(this._mode == 1){
                    if(!this.filteredName(idx, dataId)){
...
Window_BeastSelc.prototype.filteredName = function(allIdx, dataIdx){
    if(allIdx && $dataActors[allIdx]){
        if($dataActors[allIdx].name == SynrecMC.Beastiary.ListFilterName)return true;
    }
    if(dataIdx && $dataActors[dataIdx]){
