 ...
 * @target MZ
 *
 * @plugindesc Monster Capture for MZ v1.6
 *
 *@help
...
function isNum (chkNum){
    ...
    if(typeof(chkNumber) == 'number')return true;
    return false;
}
...
function chkUtilsVer (){
    const rmType = Utils.RPGMAKER_NAME;
    const rmmzVer = Utils.RPGMAKER_VERSION.split(".");
    const dVer = ("1.3.2").split(".");
    const dName = "MZ";
    if(rmType != dName)throw new Error(`Impossible RPG Maker Version Used. ${rmType} version detected, ${dName} version is required.`)
    if(rmmzVer.length !== dVer.length)throw new Error('Unknown RPG Maker MZ Version');
    for (let i = 0; i < rmmzVer.length; i++) {
        const v1 = parseInt(dVer[i]);
        const v2 = parseInt(rmmzVer[i]);
        if(isNum(v1) && isNum(v2)){
            if(v1 > v2)throw new Error('Bad RPG Maker MZ Version');
        }
    }
}
...
Game_Action.prototype.apply = function(target) {
    ...
    if(target.isAlive())SynrecGameActionApply.call(this, target);
    $gameParty.refresh();
}
...
Game_Action.prototype.checkCapture = function(target){
    ...
    const itemCaptureRate = isNum(item.meta.captureRate)? eval(item.meta.captureRate) / captureDivisor : SynrecMC.baseCapture / captureDivisor;
    if(!itemCaptureRate)return;
    if(this.checkResult(result, target))this.performCapture(target);
}

Game_Action.prototype.checkResult = function(result, target){
    result.clear();
...
Game_Action.prototype.performCapture = function(target){
    if(target.isEnemy()){
        ...
        const captureRate = (hpRateBonus + mpRateBonus + tpRateBonus + itemCaptureRate) / 4;
...
Game_BattlerBase.prototype.setGender = function(gender){
	if(gender){
		...
		if(this.isEnemy()){
            let genderArr = this.enemy().meta.genderArray;
            if(!isArray(genderArr)){
                genderArr = [];
                for(i = 0; i < SynrecMC.genders.length; i++){
                    genderArr.push(SynrecMC.genders[i]['Gender Name']);
                }
            }else{
                for(i = 0; i < genderArr.length; i++){
                    genderArr[i] = genderArr[i].toLowerCase();
                }
            }
...
Window_StatusBase.prototype.placeActorName = function(actor, x, y) {
    const key = "actor%1-name".format(actor.actorId() + $gameParty._actors.indexOf(actor));
...
Window_StatusBase.prototype.placeStateIcon = function(actor, x, y) {
    const key = "actor%1-stateIcon".format(actor.actorId() + $gameParty._actors.indexOf(actor));
...
Window_StatusBase.prototype.placeGauge = function(actor, type, x, y) {
    const key = "actor%1-gauge-%2".format(actor.actorId() + $gameParty._actors.indexOf(actor), type);
...
Window_ReserveBox.prototype.drawItem = function(index) {
    ...
    if(data){
        ...
        this.drawCharacter(charName, charIdx, x + 36 , y + 48);
    }
}
...
Window_TeamBox.prototype.drawItem = function(index) {
    ...
    if(data){
        ...
        this.drawText(gender.toUpperCase(), x + 36, y + 54);
        this.resetTextColor();
